  ...
  for (const width of [375, 768, 1024, 1440]) {
    ...
    for (const [name, path] of routes) {
      ...
      if (width === 375 || width === 1440) {
        await page.screenshot({ path: `reports/taste-review/final-${name}-${width}.png`, fullPage: true });
        if (name === 'home' || name === 'zh-home') await page.screenshot({ path: `reports/taste-review/final-viewport-${name}-${width}.png` });
      }
    }
    await page.close();
  }
  writeFileSync('reports/taste-review/measurements.json', JSON.stringify(measurements, null, 2) + '\n');
